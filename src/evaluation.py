    ...
    try:
        ...
        if dataset in ["A2D2", "KITTI", "Oxford"]:
            ...
            if dataset in ["A2D2", "KITTI"]:
                ...
                u_values_road = [u_value_road_1, u_value_road_5, u_value_road_10, u_value_road_15, u_value_road_20, \
                                 u_value_road_25]

                #regression
                model = distance_regression(road_rows_lidar, road_distances_lidar)
                max_extrap_dist_99, max_extrap_dist_95, max_extrap_dist_90, max_extrap_dist_85, max_extrap_dist_80, \
                max_extrap_dist_75 = calculate_distance_from_regression(model, u_values_road)
            ...
            meta_parameters.append(parameters)
...
for i in range(len(meta_parameters)):
    speed.append(meta_parameters[i][0])
    longitude.append(np.float(meta_parameters[i][1]))
    latitude.append(np.float(meta_parameters[i][2]))
    acceleration.append(meta_parameters[i][3])
...
np.savetxt("Results/A2D2_acceleration", acceleration, fmt="%1.5f")

d = {'timestamp': timestamps, 'max_detected': max_road_distance_detected, '99% detected': \
    ...
    max_road_distance_detected_80, '75% detected': max_road_distance_detected_75, '99% extrapolated':
    ...
    'acceleration': acceleration, 'longitude': longitude, 'latitude': latitude, 'cars 50%': cars05, 'trucks 50%': \
    trucks05, 'pedestrian 50%': ped_bc05, 'other 50%': other05, 'cars 40%': cars04, 'trucks 40%': trucks04, \
    'pedestrian 40%': ped_bc04, 'other 40%': other04, 'cars 30%': cars03, 'trucks 30%': trucks03, 'pedestrian 30%': \
    ped_bc03, 'other 30%': other03}

final_result = pd.DataFrame(data = d)
final_result.to_csv('Results/Oxford.csv', index=False)
