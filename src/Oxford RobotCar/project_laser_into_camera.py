...
def project_laser_into_camera(image_directory, laser_directory, ins_or_vo_file, camera_models_directory, extrinsics_directory, image_number):
    model = CameraModel(models_dir=camera_models_directory, images_dir=image_directory)
    extrinsics_path = os.path.join(extrinsics_directory, model.camera, '.txt')
    with open(extrinsics_path) as extrinsics_file:
        extrinsics = [float(x) for x in next(extrinsics_file).split(' ')]
    ...
    timestamps_path = os.path.join(image_directory, os.pardir, model.camera + '.timestamps')
    if not os.path.isfile(timestamps_path):
