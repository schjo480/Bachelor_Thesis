...
def select_furthest_road_pixel(semantic_image_front_center_undistorted):
    road_color = np.array([255, 0, 255])
    road_pixels = np.zeros((1300, 2))
    for i in range(semantic_image_front_center_undistorted.shape[0]):
        for j in range(semantic_image_front_center_undistorted.shape[1]):
            if (semantic_image_front_center_undistorted[i, j] == road_color).all():
                road_pixels[i, 0] = i
                road_pixels[i, 1] = j
    road_pixels = road_pixels[np.all(road_pixels != 0, axis=1)]
    return np.min(road_pixels[:, 0])


def get_y_value_of_furthes_lidar_road_pt(road_pts, lidar):
    #Compute the maximal depth of road points in m
    max_distance_road = np.max(np.linalg.norm(road_pts, axis=1))
    rows = (lidar['row'] + 0.5).astype(np.int)
    cols = (lidar['col'] + 0.5).astype(np.int)
    distance = lidar['distance']

    #Get index where distance == max_distance_road
    index = np.where(np.around(distance, 3) == np.round(max_distance_road, 3))
    x_value = cols[index]
    y_value = rows[index]
